...
class JstatData() {
    ...
    fun process(result: String): Map<String, ProcessJstat> {
        ...
        for (i in 0 until numberOfProcessesDetected) {
            val headers = lines[currentIndex].split("\\s+".toRegex()).dropLast(1)
            val value = lines[++currentIndex].split("\\s+".toRegex()).drop(1)
    ...
    private fun totalCapacity(jspMapValues: Map<String, Double>): Double {
        return jspMapValues["EC"]!! + jspMapValues["OC"]!! + jspMapValues["S0C"]!! + jspMapValues["S1C"]!! + jspMapValues["MC"]!! + jspMapValues["CCSC"]!!
    }
